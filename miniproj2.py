...
class Game:
	...
	def __init__(self, b, b_pos, s, t, d1, d2, a):
	...
	def heuristic(self, board, h):
		...
		if h == 1:		# Default heuristic
			...
			if no == nx and no < self.s:
				eval = (no * (ne+1))/(self.n - 1)
			eval = (no - nx)*(ne + 1)/(self.s - 1)
			return eval
	...
	def minimax(self, isMaximizer, depth, heuristic, board):
		result = self.is_end()
			if result == 'X':	# player 'X' wins
				...
				if depth == 0:
					return heuristic(board, heuristic)
		if isMaximizer:
			maxEval = -math.inf
			numEmptySpots = countNumEmpty(board) #find the number of empty blocks
		    for k in range (numEmptySpots):  # for each child node
				boardCopy = np.copy(board)	# make copies/number of empty blocks
				for i in range (0, self.n - 1):
					for j in range (0, self.n -1):
						if boardCopy[i][j] == '.':
							boardCopy[i][j] = 'O'
							eval = self.minimax(isMaximizer=False, depth -1, h, boardCopy)
			...
			for k in range (numEmptySpots):
				...
				for i in range (0, self.n - 1):
					for j in range (0, self.n -1):
						if boardCopy[i][j] == '.':
							boardCopy[i][j] = 'X'
							eval = self.minimax(isMaximizer=True, depth -1, h, boardCopy)
		    ...
		    for k in range (numEmptySpots):	#For each child of the node
				...
				for i in range (0, self.n - 1):
					for j in range (0, self.n -1):
						if boardCopy[i][j] == '.':
							boardCopy[i][j] = 'O'
							eval = self.alphabeta(isMaximizer=False, depth -1, h, boardCopy, alpha, beta)
							maxEval = max(maxEval, eval)
							x = i
							y = j
			...
			for k in range (numEmptySpots):
				...
				for i in range (0, self.n - 1):
					for j in range (0, self.n -1):
						if boardCopy[i][j] == '.':
							boardCopy[i][j] = 'X'
							eval = self.alphabeta(isMaximizer=True, depth -1, h, boardCopy, alpha, beta)
							minEval = min(minEval, eval)
							x = i
							y = j
							beta = min(beta, eval)
	...
	def play(self,player_x,player_o):
		...
		while True:
			...
			if self.check_end():
				return
			# start = time.time()
			if player_o.get('mode') == 'AI' and self.player_turn == 'O':
				if self.a == False:
					(_, x, y) = self.minimax(max=True, player_o.get('depth'), player_o.get('heuristic'), self.current_state)
				else:
					(_, x, y) = self.alphabeta(max=True, player_o.get('depth'), player_o.get('heuristic'), self.current_state, alpha= -math.inf, beta= math.inf)
			elif player_x.get('mode') == 'AI' and self.player_turn == 'X':
				if self.a = False:
					(_, x, y) = self.minimax(max=False, player_x.get('depth'), player_x.get('heuristic'), self.current_state)
				else:
					(_, x, y) = self.alphabeta(max=False, player_x.get('depth'), player_x.get('heuristic'), self.current_state, alpha= -math.inf, beta= math.inf)
			elif player_o == Game.HUMAN and self.player_turn == 'O':
				(x,y) = self.input_move()
			elif player_x == Game.HUMAN and self.player_turn == 'X':
				(x,y) = self.input_move()
...
def main():
	...
	algo = input('Please specify the algorithm, 0 for Minimax, 1 for Alphabeta')
		if algo == '0':
			...
			a = True
	# Setting variables for player_o and player_x if the mode is 'AI'
	if mode_x == 3:
